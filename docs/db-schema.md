...
recommended_menu length is enforced by `system_policies` key `place.recommended_menu_max_length` before persistence
short_recommendation length is enforced by `system_policies` key `place.short_recommendation_max_length` before persistence
feature_text length is enforced by `system_policies` key `place.feature_text_max_length` before persistence
address_road and address_jibun lengths are enforced by `system_policies` key `place.address_max_length` before persistence
the database column limits remain the physical upper bounds for those policy values
...
Notes
image_url length is enforced by `system_policies` key `place.image_url_max_length` before persistence
the database column limit remains the physical upper bound for that policy value
...
visit image_url length is enforced with `system_policies.visit.image_url_max_length`
...
comment content length is enforced by `system_policies` key `comment.max_length` before persistence
user activity summary counts visible comments whose deleted_at is null

5.4 community_posts

Stores free-board community posts.
...
Notes
community post title length is enforced by `system_policies` key `community.post_title_max_length` before persistence
community post content length is enforced by `system_policies` key `community.post_content_max_length` before persistence
the database column limits remain the physical upper bounds for those policy values
user activity logging records create, update, and delete actions
...
reason_text length is enforced by `system_policies` key `report.reason_text_max_length` before persistence
review_note length is enforced by `system_policies` key `report.review_note_max_length` before persistence
report follow-up reason length is enforced by `system_policies` key `report.follow_up_reason_max_length` before downstream user sanction creation
report follow-up action memo length is enforced by `system_policies` key `report.follow_up_memo_max_length` after report context is added
the database column limits remain the physical upper bounds for those policy values
...
sanction reason length is enforced by `system_policies` key `admin.sanction_reason_max_length` before persistence
...
admin action memo length is enforced by `system_policies` key `admin.action_memo_max_length` before persistence for admin user, admin place, and admin ranking/season operations that write this table
...
report.reason_text_max_length = 255
report.review_note_max_length = 255
report.follow_up_reason_max_length = 255
report.follow_up_memo_max_length = 255
admin.sanction_reason_max_length = 255
admin.action_memo_max_length = 255
Notes
...
place create/update text fields require `policy_group = place` keys `recommended_menu_max_length`, `short_recommendation_max_length`, and `feature_text_max_length`
place create/update address values require `policy_group = place` and `policy_key = address_max_length`
place create/update image URL values require `policy_group = place` and `policy_key = image_url_max_length`
report creation, processing, and follow-up actions require `policy_group = report` keys `reason_text_max_length`, `review_note_max_length`, `follow_up_reason_max_length`, and `follow_up_memo_max_length`
admin user sanction reason validation requires `policy_group = admin` and `policy_key = sanction_reason_max_length`
admin user, admin place, and admin ranking/season operation memo validation requires `policy_group = admin` and `policy_key = action_memo_max_length`
