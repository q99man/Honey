...
Run the same owner-place smoke on a real ARM64 Android device with `KAKAO_NATIVE_APP_KEY` enabled, confirming the Kakao map remains stable while the My Page owner management flow still works against the local backend through `adb reverse`.
