...
- [x] UI QA Polish (2nd round): Improved desktop panel toggle visibility by increasing button size and adding shadows; adjusted mobile sheet vertical spacing and tab contrast; unified wishlist icons across all screens; and reinforced map control borders for better contrast on bright map backgrounds.
...
- [x] Community route follow-up confirmed the existing free-board backend/API/frontend CRUD slice and connected the desktop Home side navigation community entry directly to the real `/community` route instead of the old "준비 중" map-panel placeholder path.
...
- [x] Flutter mobile release configuration & packaging: Configured internet permission and Korean app name (`허니통`) in `AndroidManifest.xml`, updated package namespace/applicationId to `com.honeytong.app`, added keystore `key.properties` dynamic loading to `build.gradle.kts`, migrated `MainActivity.kt` to the new package location, and created a build automation script (`build-mobile-release.ps1`) and deployment guide (`mobile-release-guide.md`).
...
- [x] Web Admin UI build mismatch and user-page restore completed: resolved the mismatch where `AdminShell.tsx` used the updated `AdminNavItem.labelKey` field while `AdminDashboardPage.tsx` still referenced the old `label` field, then verified `tsc -b && vite build`. Restored `HomePage.tsx` and `DetailPage.tsx` from the earlier web-to-mobile conflict point and confirmed they were recovered without merge conflicts.
...
- [x] Flutter owner place management regression coverage: User manually verified the road-address-only and jibun-address-only place detail UI no longer shows a `null` address line. Added focused mobile tests for owner place list, edit, and delete API contracts through `PlaceService`, confirming `GET /api/users/me/places`, `PATCH /api/places/{id}`, and `DELETE /api/places/{id}` are called with the expected DTO shape. Verified mobile static analysis and full Flutter test suite pass.
- [x] Flutter x86 emulator owner-place smoke: Confirmed ARM64 Android emulator images cannot boot on this x86_64 Windows host, then used the x86_64 `Pixel_10_Pro` emulator with a dev APK built without `KAKAO_NATIVE_APP_KEY` so Kakao native map initialization is skipped. Verified backend health through `adb reverse`, app launch without the Kakao native-library crash, Korean Home fallback map state, My Page login, `내 등록 맛집 관리` entry, empty owner list state, owner list item rendering from a backend-created test place, edit screen navigation with existing place data populated, delete confirmation dialog, delete success UI refresh, and `GET /api/users/me/places` returning an empty list after deletion.

- [x] Flutter mobile M3 design system foundation pass: Added a centralized Honeytong Material 3 theme under `mobile/lib/core/theme`, including brand colors, spacing/radius tokens, app bar/card/input/button/snackbar/navigation styling, and introduced a reusable `AppEmptyState` widget for Korean empty-state surfaces. Replaced the app root's inline `ThemeData` and legacy `BottomNavigationBar` styling with the shared `AppTheme.light()` and M3 `NavigationBar`. Added focused tests for the theme and empty-state widget. Verified `dart format` and `dart analyze lib test`; `flutter test` could not complete because the Flutter tool repeatedly timed out without output after the dev environment bootstrap.

- [x] Flutter Home discovery M3 polish pass: Added a reusable `HomePlaceCard` for discovery list cards and selected map place cards, then migrated the Home screen's background, search surface, category chips, FAB colors, map-key missing state, empty list state, loading indicators, login prompt action, and list/selected cards onto the shared Honeytong M3 tokens and `AppEmptyState`. Added focused widget coverage for the new home place card. Verified `dart format` and `dart analyze lib test`; `flutter test` still times out in the local Flutter tool before producing output.

- [x] Flutter place flow M3 polish pass: Added shared `AppSectionTitle` and `AppSurfaceCard` components, then migrated the place registration/edit screen's scaffold, app bar, eligibility card, form fields, location picker, franchise switch, submit action, and snackbar colors onto the Honeytong M3 tokens. Updated the place detail screen's loading/error state, dialogs, hero fallback, category/audience tags, stats, recommendation/location/comment cards, comment list, and bottom recommendation/visit actions to use shared tokens and surfaces. Also aligned the owner place management screen empty state, loading overlay, delete state, and management tiles. Removed old emulator packages (`com.example.honeytong_mobile`, `com.example.flutter_01`, `com.example.cat`) and left only `com.honeytong.app.dev` installed. Verified `dart format`, `dart analyze lib test`, and ADB package cleanup; `flutter test` remains blocked by Flutter tool timeout.

- [x] Flutter remaining tab M3 polish pass: Added shared `AppLoginRequiredState` and migrated My Page, Saved Places, and Community guest states onto it. Updated My Page profile, growth, stat, action banner, language, and verification surfaces to use Honeytong M3 tokens and shared cards. Reused `AppEmptyState` for Saved Places and Community empty states, and aligned Ranking's scaffold, header, region selector, tabs, loading, empty state, and ranking cards with shared tokens. Rechecked the emulator package list after the pass and confirmed only `com.honeytong.app.dev` remains installed. Verified `dart format`, `dart analyze lib test`, `git diff --check`, and ADB package cleanup; `flutter test` still times out in the local Flutter tool before producing output.

- [x] Flutter mobile M3 cleanup and QA audit pass: Fixed legacy mojibake Korean text left in `docs/progress.md`, removed tracked mobile root QA screenshots/log dumps (`screen.png`, tab screenshots, `window_dump*.xml`, and command log text files), added ignore rules for future local QA captures, and cleared the temporary mobile Gradle user-home cache created during verification. Reverified `dart analyze lib test`, `git diff --check`, mojibake search across mobile UI/test files and progress notes, and ADB package cleanup; the emulator still has only `com.honeytong.app.dev` installed. Full `flutter test`/Flutter build verification remains blocked in this tool session because the Flutter wrapper and Gradle build commands time out before producing useful output.

- [x] Flutter mobile build verification unblock and Home ANR diagnosis: Identified the Flutter wrapper timeout as a sandbox write-permission issue against `C:\flutter\bin\cache\lockfile`; running Flutter outside the sandbox restored `flutter --version`, `flutter test`, and dev debug APK build. Installed the APK on `emulator-5554`, confirmed only `com.honeytong.app.dev` remains installed, and captured that the app enters an Android ANR dialog shortly after launch. Logcat/DropBox evidence points to eager startup location-service work (`GeolocatorLocationService`) during Home initialization, so Home now renders immediately from the fallback center and defers GPS lookup to the current-location button with an 8-second limit. Verified the code with `dart analyze lib test`, but the post-fix `flutter test`/rebuild/reinstall verification could not be rerun because sandbox escalation was rejected by the session usage limit.

- [x] Flutter mobile post-ANR emulator QA pass: Reran `flutter test`, rebuilt the dev debug APK, installed it on `emulator-5554`, restored `adb reverse tcp:8080 tcp:8080`, and confirmed the emulator still has only `com.honeytong.app.dev` installed. Home now launches without the Android ANR dialog, keeps app focus after stabilization, and shows the Korean Kakao map configuration state. Bottom navigation was smoke-tested through the actual accessibility bounds for Home, Ranking, Saved Places, Community, and My Page; all five tabs switched successfully with no ANR, input-dispatch timeout, or fatal exception in the checked logcat window.

- [x] Flutter logged-out state M3 polish pass: Expanded the shared `AppLoginRequiredState` into a fuller M3 surface with an optional context chip and feature preview rows, then applied it to Saved Places, Community, and My Page so logged-out states explain the service value instead of feeling empty. Verified the new shared preview content with widget coverage, rebuilt the dev APK, and captured emulator screenshots for Saved Places, Community, and My Page. Tightened copy after visual QA to avoid awkward Korean line breaks. Verified `dart analyze lib test`, full `flutter test`, `git diff --check`, and mojibake search across mobile UI/tests and progress notes.

- [x] Flutter place image registration UI pass: Added an M3-style image URL editor to the mobile place registration/edit flow so users can attach existing image URLs, preview them, set representative image order, remove entries, and submit them through the existing backend `imageUrls` contract. Added focused widget coverage for the image editor and verified `dart analyze lib test` plus full `flutter test`. Real device gallery upload remains a follow-up backend/storage feature rather than a fake local-only UI.

- [x] Real image upload and profile edit foundation: Added an authenticated backend image upload API at `POST /api/uploads/images` with server-side file type/size validation, local filesystem storage, public `/uploads/images/**` serving, and URL-returning DTOs for place/profile/visit targets. Restored `users.profile_image_url` into the backend entity/profile DTO/update flow, added Flutter `image_picker` support, connected gallery upload to the place registration image list, and added a Korean My Page profile edit screen with profile image upload/removal. Verified focused backend upload/profile tests, focused Flutter upload/model tests, full backend tests, `dart analyze lib test`, full `flutter test`, dev APK build, emulator install, and emulator package cleanup with only `com.honeytong.app.dev` remaining.

- [x] Flutter place image viewing polish pass: Added a reusable place detail image gallery so multiple uploaded place images can be swiped, counted, and selected from thumbnail navigation instead of showing only the first image. Updated Home discovery cards to use the representative place image as a compact exploration thumbnail when available, while retaining the restaurant icon fallback. Verified `dart analyze lib test`, focused gallery/card widget tests, full `flutter test`, dev APK build, emulator install, and package cleanup with only `com.honeytong.app.dev` remaining.

- [x] Flutter image interaction bugfix pass: Added shared place thumbnails to the owner-place and saved-place list cards so image-backed places show their representative photo instead of an icon, normalized local uploaded image URLs against the configured API base URL for emulator/device rendering, enabled mouse and trackpad dragging through the app scroll behavior for carousel QA, strengthened the detail hero back button with a contrast-safe circular background, and expanded place/visit/profile image URL storage and policies from 255 to 2048 characters with a Flyway migration. Verified `dart analyze lib test`; Flutter/Gradle runtime tests could not be rerun in this session because the escalation budget was exhausted and sandboxed Flutter/Gradle commands timed out or attempted blocked network access.

- [x] Approved verification retry for Flutter image interaction work: Verified full `flutter test`, focused backend image/place/visit tests, dev debug APK build, and APK install on `emulator-5554`. Confirmed backend/API long image URL registration after aligning the local existing `system_policies.place.image_url_max_length` and `system_policies.visit.image_url_max_length` rows to 2048; the created place detail preserved two image URLs with a 900+ character representative URL. Physical UI E2E for list thumbnail and detail carousel mouse drag could not be completed because the Android 17 `Pixel_10_Pro` emulator produced black screenshots and repeated input-dispatch ANR/no-focused-window states during Flutter UI interaction, even after restarting the emulator as a visible window. Captured QA artifacts under `.tmp/e2e/` and ANR evidence in logcat/dropbox output.
- [x] Physical-device image interaction QA pass: Connected a Samsung `SM-A325N` physical device and confirmed the dev APK could render a backend-created long-image-URL place with a real representative thumbnail in the Home list and the first gallery image in place detail. Reproduced that thumbnail tap changed the gallery to `2 / 2` while direct carousel body drag stayed at `1 / 2`, then added explicit horizontal drag handling to the shared place image gallery and fixed the Korean thumbnail tooltip text. Verified `dart analyze lib test`, focused `place_image_gallery_test.dart`, full `flutter test`, dev debug APK build, phone reinstall, and user-confirmed physical-device list thumbnail/carousel drag behavior.
- [x] Flutter place image fullscreen viewer pass: Added tap-to-open fullscreen image viewing from the place detail gallery. The viewer opens from the currently selected image, shows a Korean close action and image counter, supports dragging between multiple images, and keeps pinch-zoom/pan behavior for the active image. Added widget coverage for open/close, current-index startup, and fullscreen drag navigation. Verified `dart format`, `dart analyze lib test`, focused `place_image_gallery_test.dart`, full `flutter test`, dev debug APK build, phone reinstall on `SM-A325N`, and `adb reverse tcp:8080 tcp:8080`.

Next task:

Run a quick physical-device smoke of the fullscreen image viewer tap, close, multi-image drag, and pinch zoom behavior, then continue to the next image-flow feature or bug found during that smoke.
