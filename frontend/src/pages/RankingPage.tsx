...
export default function RankingPage({
  ...
  useEffect(() => {
    ...
    getCurrentSeason()
      ...
      .catch(() => {
        if (mounted) {
          setErrorMessage("현재 시즌 정보를 불러오지 못했어요.");
        }
      });
  ...
  return (
    <div className="min-h-screen bg-m3-surface">
      <main className="mx-auto min-h-screen max-w-[430px] bg-m3-surface px-4 pb-24 pt-6 text-m3-on-surface">
        <header>
          <p className="text-m3-label-md text-m3-primary">랭킹</p>
            ...
            {regionOptions.map((option) => {
              ...
              return (
                <button
                  ...
                  className={
                    "shrink-0 rounded-m3-full px-4 py-2 text-m3-label-lg transition " +
                    (active
                      ? "bg-m3-primary text-m3-on-primary shadow-m3-1"
                      : "bg-m3-surface-container-lowest text-m3-on-surface-variant shadow-m3-1")
                  }
                >
                  {option.label}
                </button>
        ...
        {!isBusy && !visibleError && ranking && ranking.items.length > 0 && (
          <section className="mt-5 space-y-4">
            <div>
              <h2 className="text-m3-title-md text-m3-on-surface">상위 3곳</h2>
...
function RankingItem({
  ...
  return (
    <Link
      ...
      className={
        "block rounded-m3-xl bg-m3-surface-container-lowest p-4 text-m3-on-surface shadow-m3-1 transition active:scale-[0.99] " +
        (highlight ? "border-2 border-m3-primary" : "border border-transparent")
      }
    >
      <div className="flex items-start gap-3">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-m3-full bg-m3-primary text-m3-label-lg text-m3-on-primary">
          {item.rank}
        </div>
        <div className="min-w-0 flex-1">
          <h3 className="truncate text-m3-title-md text-m3-on-surface">
            {item.name}
          </h3>
          <p className="mt-1 text-m3-body-md text-m3-on-surface-variant">
          ...
          {item.audienceTags.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {item.audienceTags.slice(0, 3).map((tag) => (
                <span
                  key={tag}
                  className="rounded-m3-full bg-m3-secondary-container px-3 py-1 text-m3-label-md text-m3-on-secondary-container"
                >
                  {tag}
                </span>
              ))}
